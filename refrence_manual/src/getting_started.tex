...
The first command entered into most DERP sessions is the \code{load} command. The load command tells DERP to add the language
...
Selections can be executed immediately while they are being built. In order to fetch the posts specified by the current selection, we simply need to call \code{read}.
\newline\begin{minipage}{\linewidth}\begin{lstlisting}
$ python3 DERP.py
>>> load reddit
>>> create a new selection
...
\code{stop} and \code{exit} can be used to end the current DERP operation, so entering \code{stop} right now will let DERP know we want to stop building a selection, and then entering \code{stop} or \code{exit} will stop the DERP interpreter entirely.
\newline\begin{minipage}{\linewidth}\begin{lstlisting}
$ python3 DERP.py
>>> load reddit
>>> create a new selection
