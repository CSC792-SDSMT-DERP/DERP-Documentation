\section{Qualifiers}
While straightforward, the simple \code{add} and \code{remove} statements determine which results we receive with broad strokes.
...
\subsection{The \code{which} Keyword}
...
\subsection{The \code{with} Keyword}
The \code{with} keyword allows us to define a qualifier which matches results with certain fields such as strings, numbers, and dates.

\subsubsection{Query by Contained Phrase}
If we want to query for certain phrases in our posts, we can use \code{with ... in the} expressions. These expressions specify substring matches which are useful for filtering away unrelated topics.
\newline\begin{minipage}{\linewidth}\begin{lstlisting}
>>> create a new selection
...
If we know the exact title of an article, we can find it in a given source using a \code{with the exact} expression.
\newline\begin{minipage}{\linewidth}\begin{lstlisting}
>>> create a new selection
...
More generally, if we know the exact value of a given string field, we can use a \code{with the exact} expression to match against it.
...
\end{lstlisting}\end{minipage}
This makes sure that our results have 500 upvotes, but it will retrieve results with \underline{only} 500 upvotes. Usually, we would want to define a range of acceptable values for a numeric field. To accomplish this, we can use the \code{over} and \code{under} qualifiers.

The \code{over} qualifier requires that the returned posts have a numeric value greater than the value we asked for. In fact, the phrase \code{greater than} may be used instead of \code{over}.
\newline\begin{minipage}{\linewidth}\begin{lstlisting}
>>> create a new selection
...
\end{lstlisting}\end{minipage}
Assuming the current date was October 21, 2018 this would get us posts in the last day. We can also use \code{before} and \code{on} to limit ourselves to dates before and equal to the given date, respectively.



\subsubsection{Negative Qualifiers Using \code{without}}
We can also invert the meaning of a \code{with} statement by using \code{without}. For example, if we want finance posts that are about anything except savings account information we could use the following selection.
\newline\begin{minipage}{\linewidth}\begin{lstlisting}
>>> create a new selection
>>> add posts from subreddit "finance" without "savings accounts" in the body
\end{lstlisting}\end{minipage}
\code{without} is a good way to express negative logic in a qualifier, and it can allow for specifying selections without needing a remove statement.


\subsection{The \code{on} Keyword}
The \code{on} keyword makes it easy to perform a topic search. Another phrase that can be used in place of on is \code{about}. The actual functionality of \code{on} is implementation-dependent. The original desktop implementation of \code{on} performs substring searches over the "topical" fields as defined by a given module. In this case, we can think about the keyword \code{on} as shorthand for multiple \code{with ... in the} qualifiers. Let’s use the same example as in the  \code{with ... in the} section above.
\newline\begin{minipage}{\linewidth}\begin{lstlisting}
>>> create a new selection
>>> add posts from subreddit "finance" on "stocks"
\end{lstlisting}\end{minipage}
We can negate \code{on} by placing the \code{not} modifier in front of it. The previous selection is equivalent to the following.
\newline\begin{minipage}{\linewidth}\begin{lstlisting}
>>> create a new selection
>>> add posts from subreddit "finance"
>>> remove posts not on "stocks"
\end{lstlisting}\end{minipage}
\subsection{Combining Qualifiers Using \code{and} and \code{or}}
We may want to use multiple qualifiers when creating a selection. Consider the case in which we want to control what posts young children will see. We could prevent questionable content from showing up using the following query with \code{and}.
\newline\begin{minipage}{\linewidth}\begin{lstlisting}
>>> create a new selection
>>> add posts from subreddit "all" which are "verified" and which are not "NSFW"
\end{lstlisting}\end{minipage}
This statement will check for the presence of the verified flag and the absence of the NSFW tag. Alternatively, if we wanted at least one of our conditions to hold true we could replace the \code{and} with an \code{or}.

When we use \code{and} and \code{or} together we need to make sure to understand how our qualifiers will be interpreted by the runtime. The \code{and} keyword has higher precedence than the \code{or} keyword, which has implications for how we chain statements. For instance, the following selection likely has an error.
\newline\begin{minipage}{\linewidth}\begin{lstlisting}
>>> create a new selection
>>> add posts from subreddit "all" which are "verified" or which are not "NSFW" and with a post date after October 14 2018
\end{lstlisting}\end{minipage}
We can infer that we tried to find recent trustworthy posts, but, in reality, older posts may slip through if they are marked "verified". As \code{and} is higher precedence than \code{or}, the date check is and-ed with the "NSFW" check before getting or-ed with the "verified" check. To write this selection correctly we can split it up into multiple add and remove statements.
\newline\begin{minipage}{\linewidth}\begin{lstlisting}
>>> create a new selection
...
\end{lstlisting}\end{minipage}
An additional point to note is that when writing a selection, if \code{and} or \code{or} are not specified, the qualifiers are implied to have the \code{and} keyword between them.
\newline\begin{minipage}{\linewidth}\begin{lstlisting}
>>> create a new selection
