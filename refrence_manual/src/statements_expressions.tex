...
\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
    \myitem{\textit{statement}}{:} \texttt{\textit{expression} \textbf{'}\textbackslash n\textbf{'}}\\
\end{description}
...
\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
    \myitem{\textit{load\_expression}}{:} \texttt{load \textit{string}}\\
\end{description}
...
\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
    \myitem{\textit{unload\_expression}}{:} \texttt{unload \textit{string}}\\
\end{description}
...
\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
    \myitem{\textit{recall\_expression}}{:} \texttt{recall \textit{string}}\\
\end{description}
...
\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
    \myitem{\textit{clear\_expression}}{:} \texttt{clear \textit{string}}\\
\end{description}
...
\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
    \myitem{\textit{read\_expression}}{:}  \texttt{read \textit{string} \textit{sort\_predicate}\textbf{?}}\\
    \myitem{\textit{sort\_predicate}}{:}  \texttt{\textbf{(}sorted \textbf{\textbar} ordered\textbf{)} by \textit{sortkey order}\textbf{?}}\\
    \myitem{\textit{order}}{:}  \texttt{ascending \textbf{\textbar} descending}\\
\end{description}

The string given immediately after the read keyword should correspond to a previously created selection.
Any field defined by a source module is to be a valid \ecode{sortkey}, however source modules may define additional
syntax for \ecode{sortkey} to allow sorting by more context-specific means. Any posts returned from the selection
for which the \ecode{sortkey} is not relevant (for example, if multiple sources are used in the selection, and the
\ecode{sortkey} is specific to one of the sources) should be sorted to the end of the list in any order.
...
\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
    \myitem{\textit{create\_expression}}{:} \texttt{create \textit{article}\textbf{?} new\textbf{?} \textbf{(}selection \textbf{\textbar} criteria\textbf{)}}\\
\end{description}
...
criterion so far.
\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
    \myitem{\textit{recall\_expression}}{:}  \texttt{recall}\\
\end{description}
...
\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
    \myitem{save\_expression}{:} \texttt{save as \textit{string}}\\
\end{description}
...
\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
    \myitem{\textit{add\_expression}}{:}  \texttt{add posts \textit{selector}}\\
    \myitem{\textit{remove\_expression}}{:}  \texttt{remove posts \textit{selector}}\\
\end{description}
...
\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
    \myitem{\textit{selector}}{:} \texttt{\textit{qualifier\_or}}\\
    \myitem{\textit{qualifier\_or}}{:}  \texttt{\textit{qualifier\_or} or \textit{qualifier\_and} \textbf{\textbar} \textit{qualifier\_and}}\\
    \myitem{\textit{qualifier\_and}}{:}  \texttt{\textit{qualifier\_and} and \textit{qualifier} \textbf{\textbar} \textit{qualifier}}\\
\end{description}
...
\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
\myitem{\textit{Field}}{:}  \texttt{\textit{nsfw} \textbf{\textbar} \textit{upvotes} \textbf{\textbar} \textit{date}}
\end{description}}

\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
  \myitem{\textit{qualifier}}{:}  \ecode{with\_exp article field date\_check date}\\
  \myitem{}{\textbf{\textbar}}  \ecode{with\_exp string substring\_check field}\\
  \myitem{}{\textbf{\textbar}}  \ecode{boolean\_check field}\\
  \myitem{}{\textbf{\textbar}}  \ecode{string\_check field string}\\
  \myitem{}{\textbf{\textbar}}  \ecode{number\_check number field}\\
  \myitem{}{\textbf{\textbar}}  \ecode{on\_exp string}\\
  \myitem{}{\textbf{\textbar}}  \ecode{matching string}\\
\end{description}
...
\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
    \myitem{\textit{with\_exp}}{:} \texttt{with \textbf{\textbar} without}
\end{description}
...
\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
    \myitem{\textit{qualifier}}{:}  \texttt{\textit{with\_exp article field date\_check date}}\\
    \myitem{\textit{date\_check}}{:}  \texttt{date (on \textbf{\textbar} after \textbf{\textbar} before)}\\
    \myitem{\textit{date}}{:}  \texttt{\textbf{(} month day\textbf{?} \textbf{)?} year}\\
    \myitem{\textit{year}}{:}  \ecode{digit digit digit digit}\\
    \myitem{\textit{month}}{:} \texttt{january \textbf{\textbar} february \textbf{\textbar} march \textbf{\textbar} april \textbf{\textbar} may \textbf{\textbar} june \textbf{\textbar} july \textbf{\textbar} august \textbf{\textbar} september \textbf{\textbar} october \textbf{\textbar} november \textbf{\textbar} december}\\
    \myitem{\textit{day}}{:} \texttt{\textbf{[0-3]} \textit{digit}}\\
\end{description}
...
\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
    \myitem{\textit{qualifier}}{:} \ecode{with\_exp string substring\_check field}
    \ecode{substring\_check}{:} \texttt{in the \textbf{?}}
\end{description}
...
\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
        \myitem{\textit{qualifier}}{:}  \ecode{boolean\_check field }
        \myitem{\textit{boolean\_check}}{:}  \texttt{which are not\textbf{?}}
\end{description}
...
written by a specific person: \code{‘without the exact author “fred flintstone”’}. Note, this would require that the source module which
generated the post populates the field ‘author’
\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
          \myitem{\textit{qualifier}}{:}  \ecode{string\_check field string}
          \myitem{\textit{string\_check}}{:}  \texttt{\textit{with\_exp} the exact}
\end{description}
...
\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
        \myitem{\textit{qualifier}}{:}  \texttt{\textit{number\_check number field}}\\
        \myitem{\textit{number\_check}}{:}  \texttt{\textit{with\_exp}\textbf{?} exactly \textbf{\textbar} \textit{above\_expression} \textbf{\textbar} roughly}\\
        \myitem{\textit{above\_expression}}{:}  \texttt{over \textbf{\textbar} greater than}\\
        \myitem{\textit{below\_expression}}{:}  \texttt{under \textbf{\textbar} less than}\\
\end{description}
The keyword exactly, and expressions \ecode{above\_expression} and \ecode{below\_expression} are mapped to the traditional mathematical operations ==, $>$, and $<$, respectively.
...
Note that defining the \ecode{number\_check} production using \ecode{with\_exp} provides some additional flexibility in how qualifiers can be used to match numbers. To match a
number in a way analogous to the standard >= operation, DERP code could specify the single qualifier \code{‘without less than x field’}; however, it may be more natural
...
the same result.
\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
      \myitem{\textit{qualifier}}{:} \texttt{\textit{on\_exp string}}\\
      \myitem{\textit{on\_exp}}{:} \texttt{on \textbf{\textbar} about}\\
\end{description}
...
\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
    \myitem{\textit{qualifier}}{:}  \texttt{matching \textit{string}}\\
\end{description}
...
As mentioned above, selection mode is very similar to criteria mode, but has extra capabilities. While \ecode{add\_expressions} and \ecode{remove\_expressions} in Criteria Mode
...
\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
    \myitem{\textit{selector}}{:} \texttt{\textit{qualifier\_or} \textbf{\textbar} from \textit{source qualifier\_or}\textbf{?}}
    \myitem{\textit{source}}{:} \texttt{\textit{source} \textbf{(} and \textbf{|} or \textbf{)} \textit{source}}
\end{description}
...
Because \ecode{remove\_expressions} in selection mode are no longer limited to the fields of posts, but can also remove posts based solely on their origin, ambiguities
...
the following semantic rule: \ecode{remove\_expression} is valid in Selection mode iff
\begin{itemize}
\item It does not contain any \ecode{source}, and at least one \ecode{add\_expression} has been executed after selection mode was entered, but before the \ecode{remove\_expression} in question.
\item OR, for every source in the \ecode{remove\_expression} that matches syntax defined by a source module, that source was used in an \ecode{add\_expression} after selection mode was entered, but before the \ecode{remove\_expression} in question.
\end{itemize}
As an example, this set of statements is valid in Selection Mode…\footnote{Provided that the following syntax has been added\\
\begin{description}[labelindent=1cm,leftmargin=\onelen,labelwidth=1cm]
\myitem{\textit{source}}{:}  \texttt{cnn \textbf{\textbar} fox}
\end{description}}
\begin{lstlisting}
