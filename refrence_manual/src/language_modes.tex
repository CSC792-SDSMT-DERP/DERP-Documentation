...
Use a \nonterminal{create\_expression} with the \terminal{selection} keyword to switch from Main Mode to Selection Mode
\begin{itemize}[leftmargin=2in]
    \item[\nonterminal{create\_expression}] \bnf{:} \terminal{create} \nonterminal{article}\bnf{?} \nonterminal{new}\bnf{?} \terminal{selection}
\end{itemize}

Use a \nonterminal{create\_expression} with the \terminal{criteria} keyword to switch from Main Mode to Criteria Mode
\begin{itemize}[leftmargin=2in]
...
it is a filter that can be applied to a selection, but it is not a valid selection on its own. (And, therefore, using \nonterminal{read\_expression} with a criterion is not valid)
